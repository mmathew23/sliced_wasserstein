# https://github.com/biweidai/SINF/blob/master/sinf/fid_score.py
...
def get_activations(images, model, batch_size=50, dims=2048, device='cpu'):
    ...
    dl = torch.utils.data.DataLoader(images, batch_size=batch_size,
                                     drop_last=False, shuffle=False, num_workers=12)
...
def evaluate_fid_score(images1, images2, batch_size=50, device='cuda:2'):
    """Calculation of FID.
    Params:
    ...
    if torch.max(images1) > 1 or torch.min(images1) < 0 or torch.max(images2) > 1 or torch.min(images2) < 0:
        import warnings
        warnings.warn('FID score: the values of images should be in range [0,1].')
    
    # print(images1.mean(), images2.mean())
